	...
	repeat with i from 1 to count of selectedVM_URLs
		...
		repeat until selectedVM_URL_CURL_PID = ""
			...
			if selectedVM_URL_CURL_PID ­ "" then
				...
				try
					-- Display progress
					display dialog "Downloading file " & i & " of " & (count of selectedVM_URLs) & "É" & return & "Total size: " & selectedVM_URL_sizeMB & "MB" & return & "Downloaded: " & selectedVM_URL_currentSizeMB & "MB (" & selectedVM_URL_currentSizePercentage & "%)" buttons {"Cancel"} cancel button "Cancel" with title "modern.IE VM Installer" giving up after 1
				on error
