...
if allVMs contains selectedVM then
	...
	repeat with i from 1 to count of selectedVM_URLs
		...
		repeat until selectedVM_URL_CURL_PID = ""
			...
			if selectedVM_URL_CURL_PID ­ "" then
				...
				-- Display progress
				GrowlNotify("Downloading file " & i & " of " & (count of selectedVM_URLs) & "É", "Downloaded " & selectedVM_URL_currentSizeMB & "MB of " & selectedVM_URL_sizeMB & "MB (" & selectedVM_URL_currentSizePercentage & "%)")
			end if
	...
	if cancelled = 0 then
		...
		if OVA_VMs contains selectedVM then
			...
			-- Extract the ZIP
			GrowlNotify("Extracting ZIP fileÉ", "")
			...
			GrowlNotify("Extracting SFX fileÉ", "")
		...
		if selectedVM_OVA_pathFound = "Found" then
			...
			try
				...
				GrowlNotify("Importing OVA file into VirtualBoxÉ", "")
